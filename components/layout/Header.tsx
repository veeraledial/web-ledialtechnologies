    ...
    <header className="sticky top-0 z-50 w-full border-b border-[var(--color-light)]/10 bg-[var(--brand-footer)]">
      <div className="mx-auto flex h-16 max-w-[1200px] items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          {MAIN_NAV.map((item) =>
            item.children ? (
              ...
              >
                ...
                >
                  {item.label}
                  <svg className="h-4 w-4 transition transform group-hover:rotate-180" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
                  </svg>
                </button>
                {openDropdown === item.label && (
                  <div className="absolute left-0 top-full pt-1">
                    <div className="min-w-[220px] rounded-xl border border-[var(--border)] bg-[var(--color-light)] py-2 shadow-xl">
        ...
        <div className="hidden items-center gap-3 md:flex">
          <Link href="/contact">
            <Button variant="primary" size="sm" className="shadow-[var(--shadow-glow-red)] hover:shadow-[var(--shadow-glow-red-hover)]">
              Get Quote
            </Button>
          </Link>
        </div>
